...
def _display_name(slug):
    """'da-pan' -> 'Da Pan'  (for JSON keys and print output)"""
    return slug.replace('-', ' ').title()

def _dir_name(slug):
    """'da-pan' -> 'Da-Pan'  (for directory/file paths — hyphen kept)"""
...
class WarfarinWikiScraper:
    ...
    def scrape_operator_page(self, operator_name, url):
        print(f"Scraping {_display_name(operator_name)}...")
    ...
    def download_images(self, operator_name, soup):
        ...
        """
        images_dir = f"../Data/Images/Operators/{_dir_name(operator_name)}"
        ...
        for label, url in targets.items():
            ...
            try:
                ...
                filename = f"{_dir_name(operator_name)}_{label}.{ext}"
    ...
    def scrape_all_operators(self):
        ...
        for op in self.get_all_operator_links():
            try:
                ...
                data[_display_name(op['name'])]['images'] = self.download_images(op['name'], soup)
